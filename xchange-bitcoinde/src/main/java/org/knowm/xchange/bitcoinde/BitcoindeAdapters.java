...
public final class BitcoindeAdapters {
	...
	public static OrderBook adaptOrderBook(BitcoindeOrderbookWrapper bitcoindeOrderbookWrapper,
		...
		List<LimitOrder> asks = createOrders(currencyPair, Order.OrderType.ASK,
				bitcoindeOrderbookWrapper.getBitcoindeOrders().getAsks());
		List<LimitOrder> bids = createOrders(currencyPair, Order.OrderType.BID,
  ...
  public static LimitOrder createOrder(CurrencyPair currencyPair, BitcoindeOrder bitcoindeOrder, Order.OrderType orderType, String orderId,
	...
	public static Trades adaptTrades(BitcoindeTradesWrapper bitcoindeTradesWrapper, CurrencyPair currencyPair) {
		...
		for (BitcoindeTrade bitcoindeTrade : bitcoindeTradesWrapper.getTrades()) {
			...
			trades.add(new Trade(null, bitcoindeTrade.getAmount(), currencyPair, bitcoindeTrade.getPrice(),
                    DateUtils.fromSecondsToZonedDateTime(bitcoindeTrade.getDate()), String.valueOf(tid)));
		}
	...
	public static OpenOrders adaptOpenOrders(BitcoindeMyOpenOrdersWrapper bitcoindeOpenOrdersWrapper) {
		...
		for (BitcoindeMyOrder bitcoindeMyOrder : bitcoindeMyOrders) {
			...
			ZonedDateTime timestamp = fromRfc3339DateStringQuietly(bitcoindeMyOrder.getCreatedAt());
	...
	private static ZonedDateTime fromRfc3339DateStringQuietly(String timestamp) {
		try {
			DateTimeFormatter simpleDateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ssXXX");
			return ZonedDateTime.parse(timestamp, simpleDateFormat);
