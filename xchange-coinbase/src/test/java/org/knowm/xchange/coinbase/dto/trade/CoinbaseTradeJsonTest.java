...
public class CoinbaseTradeJsonTest {
  ...
  public void testDeserializeTransfers() throws IOException {
    ...
    assertThat(transfer.getBtcAmount()).isEqualToComparingFieldByField(new CoinbaseMoney("BTC", new BigDecimal("1.20000000")));
    assertThat(transfer.getSubtotal()).isEqualToComparingFieldByField(new CoinbaseMoney("USD", new BigDecimal("905.10")));
    assertThat(transfer.getTotal()).isEqualToComparingFieldByField(new CoinbaseMoney("USD", new BigDecimal("914.30")));
